#!/bin/sh
# Darwin/arm64 QuartzCore interface populate (ObjC: Core Animation + friends).
#
# Must use -x objective-c. FILTER_FFI_MACROS=frameworks keeps parse tractable.
#
#   cd $CCL/darwin-arm64-headers/quartzcore/C
#   $CCL/tools/darwin-arm64-cdb/quartzcore-populate.sh
#   cd $CCL
#   CCL_INTERFACE=quartzcore ./darm64cl --stack-size 16M --thread-stack-size 16M \
#     --no-init --batch < tools/darwin-arm64-cdb/parse-interface-dir.lisp
#
# BACK UP quartzcore/*.cdb first.
...
echo ";; quartzcore-populate done under $(pwd)"
