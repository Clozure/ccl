...
4:      __(ldr gpr32(imm0),[arg_y,imm2])
        __(ldr gpr32(imm1),[arg_z,imm2])
        __(cmp imm0,imm1)
        __(bne 9f)
        __(add imm2,imm2,#4)
...
_spentry(builtin_length)
        ...
        __(b.ne 0f)
        __(ldr arg_z,[arg_z,#vectorH.logsize])
        __(ret)
0:      __(b.lo 1f)
        __(vector_length(arg_z,arg_z,imm0))
        __(ret)
1:      __(branch_if_not_list(arg_z,8f))
...
2:      __(cmp temp0,rnil)
        ...
        __(b.eq 9f)
        __(branch_if_not_list(temp0,8f))
        __(_cdr(temp0,temp0))
...
_spentry(builtin_seqtype)
        ...
        __(mov arg_z,rnil)
        __(ret)
0:      __(branch_if_not_list(arg_z,1f))
        __(mov arg_z,rt)
        __(ret)        
1:      __(jump_builtin(_builtin_seqtype,1))

/* This is usually inlined these days */
_spentry(builtin_assq)
        __(b 2f)
1:      __(trap_unless_list(arg_z))
        __(_car(arg_x,arg_z))
        __(_cdr(arg_z,arg_z))
        __(cmp arg_x,rnil)
        __(beq 2f)
        __(trap_unless_list(arg_x))
        __(_car(temp0,arg_x))
        __(cmp temp0,arg_y)
        __(bne 2f)
        __(mov arg_z,arg_x)
...
_spentry(makes64)
        ...
        __(cmp imm0,arg_z,asr #fixnumshift)
        __(b.eq 1f)
...
_spentry(makeu64)
        __(mov imm1,#0xf000000000000000)
	__(tst imm0,imm1)
	__(box_fixnum(arg_z,imm0))
	__(b.eq 1f)
...
_spentry(check_fpu_exception)
        ...
        __(mov imm1,#make_header33,subtag_u32_vector)))
...
1:
        ...
        __(ret)
2:
        __(mov imm0,#make_header1,subtag_u32_vector)))
        __(mov imm1,#0)
...
_spentry(makestackblock)
        ...
        __(mov imm1,#make_headermacptr.element_count,subtag_macptr)))
...
1:
        ...
        __(bne 2f)
        __(mov imm0,#make_header1,subtag_simple_vector)))
        __(mov imm1,#0)
        __(stmdb sp!,{imm0,imm1})
        __(b 9f)
2:
