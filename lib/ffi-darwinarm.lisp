...
(defun arm-darwin::record-type-returns-structure-as-first-arg (rtype)
  (when (and rtype
...
(defun arm-darwin::expand-ff-call (callform args &key (arg-coerce #'null-coerce-foreign-arg) (result-coerce #'null-coerce-foreign-result))
    ...
    (multiple-value-bind (result-type error)
      ...
      (collect ((argforms))
        (when (typep result-type 'foreign-record-type)
          ...
          (if (arm-darwin::record-type-returns-structure-as-first-arg result-type)
            (progn
...
(defun arm-darwin::generate-callback-bindings (stack-ptr fp-args-ptr argvars argspecs result-spec struct-result-name)
...
(defun arm-darwin::generate-callback-return-value (stack-ptr fp-args-ptr result return-type struct-return-arg)
  (declare (ignore fp-args-ptr))
