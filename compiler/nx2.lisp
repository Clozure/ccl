      ...
      (destructuring-bind (req opt rest keys &rest ignore) (cdr alambda)
        ...
        (when (or (dolist (sp (caddr opt))
          ...
          (flet ((info-for-var (var)
                   (or (assoc var entries :test #'eq)
                       (error "Missing info for var ~s" (var-name var)))))
            (let* ((new ()))
              (setq entries (nreverse entries))
              (dolist (var (afunc-inherited-vars afunc))
                (push (info-for-var var) new))
              (dolist (r req)
                (push (info-for-var r) new))
              (dolist (o (car opt))
                (push (info-for-var o) new))
              (when (consp rest)
                (setq rest (car rest)))
              (when rest
                (push (info-for-var rest) new))
              (dolist (k (cadr keys))
                (push (info-for-var k) new))
              (dolist (e entries)
