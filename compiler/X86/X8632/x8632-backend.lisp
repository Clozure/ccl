...
;;; FFI stuff.  Shared by several backends (Darwin notably excepted.)

;;; A returned structure is always passed as a "hidden" first argument.
...
(defun x8632::expand-ff-call (callform args
  ...
  (let* ((result-type-spec (or (car (last args)) :void))
	...
	(do* ((args args (cddr args)))
	  ...
	  (funcall result-coerce result-type-spec
		   `(,@callform ,@(argforms)))))))
...
;;;  to STACK-PTR.  (This is unused on x8632.)
  ...
  (collect ((lets)
	...
	(setq argvars (cons struct-result-name argvars)
      ...
      (do* ((argvars argvars (cdr argvars))
	    ...
	    (offset 8))
	   ((null argvars)
	    (values nil (lets) (dynamic-extent-names) nil rtype nil 4))
