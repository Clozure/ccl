...
;;; Instruction-level round-trip tests.
;;;
;;; The invariant: assembling a LAP form yields a word; disassembling that
;;; word back to operand structs and re-encoding them must reproduce the
;;; same word.  Because both directions read the *same* template table,
;;; this checks that every mask and bit position agrees with itself -- they
;;; are hand-asserted otherwise.

(defun template-branches-p (template)
  ;; A branch's displacement decodes to a bare PC-relative offset, which
  ;; can't be re-encoded without the label machinery, so we skip these.
  (some #'label-spec-p (instruction-template-operand-specs template)))

(defun reencode-word (word)
  ;; WORD -> find its template -> decode each operand -> re-encode.
  ;; Returns the re-encoded word, or :branch for a branch (skipped), or
  ;; NIL if no template matches WORD at all.
  (let ((template (find-template word)))
    (cond
      ((null template) nil)
      ...
      (t (let ((insn (%make-instruction nil)))
           ...
           (instruction-word insn))))))

(defun assemble-word (form)
  ;; Assemble a single LAP FORM (no section) and return its 32-bit word.
  (instruction-word (assemble-instruction nil form)))

(defun check-word (word)
  ;; T if WORD survives disassemble -> re-encode unchanged (branches, which
  ;; we don't round-trip, count as passing).  Handy for probing raw words
  ;; or an external corpus.
  (let ((reencoded (reencode-word word)))
    (or (eq reencoded :branch) (eql reencoded word))))

;;; A starter corpus spanning the operand flavors.  Extend freely:
;;; run-round-trip-tests reports any form it can't assemble as an ERROR
;;; rather than crashing, so unsupported entries are informative, not fatal.
(defparameter *round-trip-forms*
  '(;; add/sub immediate
    (add x0 x1 (:$ 42))
    (sub x12 x12 (:$ 8))
    (add x0 x1 (:$ 1 :lsl 12))
    ...
    (orr x0 xzr (:$ 2))
    (and w3 w4 (:$ #xff))
    (eor x5 x6 (:$ #xf0f0f0f0f0f0f0f0))
    ;; move wide
    (movz x0 (:$ #x1234))
    (movk x5 (:$ #xabcd :lsl 16))
    (movn x7 (:$ 0))
    ;; shifted register
    (orr x0 x1 (x2 :lsl 3))
    (orr x9 x10 (x11 :ror 7))
    ;; extended register
    (add x0 x1 (w2 :uxtw 2))
    (add x0 x1 (x2 :sxtx 0))
    ;; bitfield
    ...
    (ccmp x0 x1 (:$ 3) (:? ge))
    (ccmn x0 (:$ 5) (:$ 0) (:? lt))
    ;; memory: scaled, pre/post-index, register offset, bare
    (ldr x0 (:@ x1 (:$ 16)))
    (str w2 (:@ x3 (:$ 4)))
    (ldr x0 (:@! x1 (:$ 8)))
    (ldr x0 (:@+ x1 (:$ 8)))
...
(defun run-round-trip-tests (&optional (forms *round-trip-forms*))
  ;; Assemble each FORM, round-trip its word, and report mismatches.  A
  ;; form that won't assemble (or whose decode signals) is reported as an
  ;; ERROR and the run continues.
  (let ((passed 0) (failed 0) (errored 0) (skipped 0))
    (dolist (form forms)
      (handler-case
          (let* ((word (assemble-word form))
                 (reencoded (reencode-word word)))
            (cond
              ((eq reencoded :branch) (incf skipped))
              ((null reencoded)
               (incf failed)
               (format t "~&FAIL ~s: no template re-matched #x~8,'0x~%"
                       form word))
              ((= reencoded word) (incf passed))
              (t (incf failed)
                 (format t "~&FAIL ~s: assembled #x~8,'0x, re-encoded #x~8,'0x~%"
                         form word reencoded))))
        (error (c)
          (incf errored)
          (format t "~&ERROR ~s: ~a~%" form c))))
    (format t "~&~d passed, ~d failed, ~d errored, ~d skipped (branches)~%"
...
;;; with the class marker (:CORE / SIMD) at index 5.  For each well-formed,
;;; non-alias CORE row we run find-template on the opcode and require the
;;; matched template's mnemonic to equal binutils'.  Unlike round-trip
;;; testing -- which only checks our encoder against our own decoder -- this
;;; checks us against an independent source of truth, so it finds shared
;;; errors a self-consistent round-trip would miss.

(declaim (special *binutils-data*))

(defun binutils-core-row-p (row)
  ;; A well-formed CORE row we can probe.  SIMD rows (class SIMD) and the
  ;; table's transcription typos fail these guards and are skipped.
  (and (consp row)
       (>= (length row) 9)
       (stringp (nth 0 row))
       (integerp (nth 1 row))
...
(defun binutils-alias-row-p (row)
  ;; True if the row's flags name F-ALIAS: the row is itself an alias, which
  ;; the disassembler resolves to its base, so find-template will correctly
  ;; return a different mnemonic.  (F-HAS-ALIAS marks a *real* instruction
  ;; that merely has an alias, and must not be skipped.)
  (labels ((alias-p (x)
             (cond ((symbolp x) (string-equal (symbol-name x) "f-alias"))
                   ((consp x) (or (alias-p (car x)) (alias-p (cdr x))))
                   (t nil))))
    (alias-p (nth 8 row))))

(defparameter *binutils-mismatch-whitelist*
  ;; (opcode . binutils-name): cases where binutils' table uses a parametric
  ;; parent and we (correctly) specialize, so the name "mismatch" is benign.
  '((#x54000000 . "b.c")        ;cond branch: we emit b.eq..b.nv
    (#xd503201f . "hint")))     ;hint space: we emit nop and friends

(defun binutils-mismatch-whitelisted-p (opcode name)
  (let ((entry (assoc opcode *binutils-mismatch-whitelist*)))
    (and entry (string-equal (cdr entry) name))))

(defun sweep-binutils-data (&optional verbose)
  ;; Probe the CORE, non-alias rows of *binutils-data*.  Reports MISMATCH (we decode
  ;; the opcode as a different instruction -- a real bug) and MISSING (no
  ;; template -- a coverage gap), and tallies OK.  Known-benign mismatches
  ;; (see *binutils-mismatch-whitelist*) are counted separately and not flagged,
  ;; so a genuine new mismatch stands out.
  (let ((ok 0) (mismatch 0) (missing 0) (skipped 0) (whitelisted 0))
    (dolist (row *binutils-data*)
      (cond
        ((not (binutils-core-row-p row)) (incf skipped))
        ((binutils-alias-row-p row) (incf skipped))
        (t (let* ((name (nth 0 row))
                  (opcode (nth 1 row))
                  (template (find-template opcode)))
             (cond
               ((null template)
                (incf missing)
                (format t "~&MISSING ~a #x~8,'0x~%" name opcode))
               ((string-equal (instruction-template-name template) name)
                (incf ok)
                (when verbose (format t "~&OK ~a~%" name)))
               ((binutils-mismatch-whitelisted-p opcode name)
                (incf whitelisted))
               (t
                (incf mismatch)
                (format t "~&MISMATCH #x~8,'0x: binutils ~a, ours ~a~%"
                        opcode name (instruction-template-name template))))))))
    (format t "~&~d ok, ~d mismatch, ~d missing, ~d skipped, ~d whitelisted~%"
            ok mismatch missing skipped whitelisted)
    (values ok mismatch missing skipped whitelisted)))

(defparameter *binutils-data*
'(
  ...
  ("subs" #xf1000000 #xff000000 :addsub-imm 0 :CORE '(:Rd :Rn-SP :AIMM) '(:x :x :aimm) F-HAS-ALIAS)
  ...
  ("eon" #xca200000 #xff200000 :log-shift 0 :CORE '(:Rd :Rn :Rm-SFT) '(:x :x :x-shift) 0)
