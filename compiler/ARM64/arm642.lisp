...
(defun arm642-double-float-bits (the-df)
  (multiple-value-bind (high low) (double-float-bits the-df)
      ...
      (if (and (= (hard-regspec-class vreg) hard-reg-class-fpr)
        ...
        (if (zerop form)
          ...
          (if (typep form 'short-float)
            ...
            (let* ((bits (arm642-double-float-bits form)))
              (declare (integer bits))
              (with-imm-temps () ((bitsreg :u64))
                (! lri bitsreg bits)
                (! load-double-float-constant vreg bitsreg)))))
