...
;;; ============ ref-constant â€” DROPPED (re-sync 2026-07-14) ============
...
;;; off the fulltag-function-tagged fn (constants @ slot 1; no entrypoint
;;; slot, no +2 skip). Our former definition here encoded the deleted
;;; entrypoint-slot layout and would collide â€” removed.
...
;;; equivalently, as his live popj minus the final (ret) â€” his popj already
...
;;; arg/imm/temp-with-ABI-meaning â€” v2 cont-71 class; temp0 is dead at
...
;;; Non-tail form of jump-known-symbol (same layout evidence â€” see above);
;;; result convention: value returns in arg_z, matching the emit site
;;; (! call-known-symbol arm64::arg_z)
;;; (arm642-additions.lisp:526).
...
;;; 32-bit ARM's one-instruction (ldr pc ...) trick has no arm64 analog â€”
...
;;; site calls (! call-known-function) with 0 vregs â€” the wired-arg_z
...
;;; Reconcile template ordinals with the assembler's table (same epilogue
;;; as his arm64-vinsns.lisp).
(fixup-arm64-vinsn-templates)
...
;;; [fixnumshift, fixnumshift+32) â€” ubfx. sbfx/ubfx are LAPMACROS in his
;;; tree (arm64-lapmacros.lisp:18), NOT instruction templates, and vinsn
;;; bodies don't expand lapmacros â€” write the underlying UBFM
...
;;; (la sp amount sp) â€” amount is SIGNED (:s16const). AArch64
;;; add/sub-immediate is unsigned 12-bit; a negative amount must become
;;; SUB (the s61/s72/s89 negative-immediate truncation class) â€” split on
...
;;; BASE = RCONTEXT (boot- root): the sptab lives in each thread's
...
;;; n*8)]; blr`). His call-subprim-1/2 vinsns still say rnil â€” STALE:
...
;;; would destroy the argument â€” the exact v2 cont-71 bug class
...
;;; ppc64-vinsns.lisp:3457: (tdlgti nargs (ash max word-shift)) â€”
...
;;; (tdllti nargs (ash min word-shift)) â€” unsigned trap-if-less.
...
;;; (canonical-nil #x1300b, and on Darwin not even a link-time constant â€”
...
;;; PROMOTED (2026-07-28) into upstream patch 0061 together with
...
;;; fixnum-tagged = count<<3 = the same byte offset â€” the v2 donor's
...
;;; hidden cross-vinsn channel the allocator cannot see â€” and the
...
;;; ( unwired-temp class, third instance).
...
;;; idiom); cmpldi/bne â†’ cmp/b.ne (one NZCV, no crf temp);
;;; stdu -8 vsp â†’ str (:@! vsp -8) (his vpush-register idiom);
;;; li arg_z nil â†’ mov arg_z rnil (his load-nil idiom).
...
;;; bgt(unsigned cmpldi) â†’ b.hi, beq â†’ b.eq. nargs âˆˆ [min, min+2]
...
;;; branches share that NZCV), then RE-COMPARE for crfy â€” the donor's
...
;;; (define-fixedsized-object macptr, arm64-arch.lisp) â€” not 8-aligned â‡’
...
;;; a SMALL constant (count<<8|subtag â€” the v2 Layout-T movz/movk wide-
...
;;; drops out). tdlt â†’ cmp/b.hs/uuo-alloc-trap (his uuo canon:
;;; arm64-uuo.s alloc trap; the mnemonic is in his template table).
;;; Header store at -4, value store at +4: both unaligned â‡’ STUR.
;;; clrrdi ntagbits â†’ AND with the UNSIGNED complement of fulltagmask
;;; (logical immediates are unsigned â€” his msg-18/19 rule).
...
;;; CLEARED to stay a fixnum (clrrdi) â€” AND with the unsigned complement
;;; of fixnummask. sradi./blt â†’ asr + cmp #0 + b.lt (one NZCV).
;;; count temp is :s64 (X width) â€” register-form shifts are X-form with
;;; the 64-bit num.
...
;;; misc-data-offset = -4 â‡’ LDUR, D form â€” exactly his get-double-float
...
;;; tcr.single-float-convert scratch (PPC has no GPRâ†’FPR move).
...
;;; ARMv8 FMOV (general, Wâ†’S) replaces the PPC memory bounce (same
...
;;; x8664-vinsns.lisp:1453 (little-endian, fixnumshift=3 â€” Matt's exact
;;; configuration; PPC64's ppc64-vinsns.lisp:3045 rotldi/xoris trick is
;;; big-endian-only, the donor's declared deviation, kept):
;;; btcq 63 / sarq fixnumshift / btcq 60 â€” un-wrap the overflowed
...
;;; btc â†’ EOR with a single-bit logical immediate. The v2 donor OMITTED
...
;;; ldr entry off rnil / blr) â€” see U1 for the ldr-vs-add open question
...
;;; PROPOSED extension file does â€” U4). Scratch is imm1, NOT imm0:
...
;;; and arg_y = flags â€” imm0 is LIVE, scratch must be imm1 (U2).
...
;;; low byte, charcode at charcode-shift=8 â€” the PPC64 layout exactly, so
...
;;; â‡’ value < 2^29, two lanes suffice). ori/oris â†’ movz/movk (his
...
;;; little-endian (LOW half at the lower address â€” PPC is BE) and has no
...
;;; selects the D-form LDR (offset 0 is aligned â‡’ scaled forms encode).
...
;;; bounces through the stack (no GPRâ†’FPR move on PPC). ARMv8 FMOV
;;; (general, Wâ†’S) is the direct move â€” donor's established deviation,
;;; kept. src holds the raw IEEE-754 single bits; class t carries no
;;; width â‡’ the W view is forced explicitly, selecting his
...
;;; (la dest offset vsp) â€” offset SIGNED. Same negative-immediate split
...
;;; dest, second chains off dest â€” the add-immediate contract).
...
;;; ============ make-vcell ============
;;; Donor: vinsn-retrofit-queue.lisp:434; PPC64 ppc64-vinsns.lisp:2082:
;;; li header value-cell-header / la allocptr (- fulltag-misc size) /
;;; tdlt / std header misc-header-offset / mr dest allocptr /
;;; rldicr allocptr (untag) / std closed value-cell.value(dest)
;;; Direct low-tag line-port (same pattern as double->heap): small
;;; header constant, fulltag-misc baked by the sub, AND-untag; both
;;; object stores unaligned (+/-4) â‡’ STUR. PPC's store-closed-last
;;; order kept (the (:ne dest) constraint guards the dest/closed alias;
;;; closed is stored through dest AFTER dest is written, which is safe
;;; because closed â‰  dest).
(define-arm64-vinsn make-vcell (((dest :lisp))
  ...
  (and allocptr allocptr (:$ (logand #xffffffffffffffff
                                     (lognot arm64::fulltagmask))))
  (stur closed (:@ dest (:$ arm64::value-cell.value))))


;;; ============ misc-set-c-node ============
;;; Donor: vinsn-retrofit-queue.lisp:469; PPC64 ppc64-vinsns.lisp:440:
;;; (std val (+ misc-data-offset (ash idx 3)) v). Offset = 8*idx - 4 â‰¡ 4
;;; (mod 8) â€” never 8-aligned â‡’ STUR (unscaled simm9; mirror of his
;;; misc-ref-c-node's ldur). simm9 bounds idx â‰¤ 31 (4+8*31 = 252 â‰¤ 255);
...
;;; slot 3 â€” the l1-clos not-callable). HISTORY: that was the
...
;;; ppc64-vinsns.lisp:3931: (ba .SPnvalret) â€” a JUMP, not a call.
...
;;; and pre-scales nargs â€” imm0 LIVE â‡’ imm1 scratch (U2).
...
;;; Reconcile template ordinals with the assembler's table (same epilogue
;;; as his arm64-vinsns.lisp and the additions file).
(fixup-arm64-vinsn-templates)
...
;;; PPC64 test-fixnum ppc64-vinsns.lisp:4200 (andi. tag x fixnummask â€”
...
;;; logical immediate; Z=1 iff fixnum â€” what cond-eq consumes. The
...
;;; ppc64-vinsns.lisp:2131: (ba spno) â€” a JUMP, not a call. ARM32
;;; arm-vinsns.lisp:2090 (spjump spno). v2's body (arm64-vinsns.lisp:
;;; 3305) dispatched off rcontext's sptab â€” v2-only machinery, reverts
...
;;; ============ compare ============
;;; Demand: w1-report item 2 ((crf) (x :lisp) (y :lisp) â€” cmp x,y);
...
;;; pseudo â€” cmp writes NZCV implicitly; the slot links the dep DAG to
...
;;; Demand: w1-report item 3 â€” const is the PRE-SHIFTED boxed value,
...
;;; sites arm642-additions-w1.lisp:191/:259/:262/:335 â€” the %izerop
...
;;; ============ compare-immediate ============
;;; Demand (, pin advance to 33e61e6): HIS arm642-compare,
...
;;; â€” clears the top (1+ nbits) bits, KEEPING the low (63 - nbits) bits;
...
;;; a template (w1 %unbox-u32 lesson) â€” write the underlying UBFM:
...
;;; always covers the tag bits â€” callers pass (+ fixlen fixnumshift)
;;; >= 4, so the result stays a fixnum); emit sites :413/:537. PPC64
;;; clear-right ppc64-vinsns.lisp:2870: (rldicr dest src 0 (:apply -
;;; 63 nbits)) â€” keep the top (64 - nbits) bits. One AND with the
...
;;; any nbits in 1..63 (nbits=0 would be all-ones â€” unencodable â€” and
...
;;; Demand: w1-report item 13 ((crf) (fixnum :lisp) (bit :u8const)) â€”
...
;;; â€” carried verbatim (single-bit masks are always encodable logical
...
;;; (shifted-register forms take constant shifts only) â€” materialize
...
;;; 63 to... the AArch64 register LSL amount is taken MOD 64 â€” see U5
...
;;; Demand: w1-report item 15 â€” the twin of his LIVE
...
;;; ============ add-immediate ============
;;; Demand: w1-report item 16 â€” SHIFTED constant in (signed-byte 24),
;;; body emits 1-2 ADD/SUB immediates, alias-safe for dest = src (the
;;; w1 U5w contract; emit site :712 gates to signed-byte 24). PPC64
;;; add-immediate ppc64-vinsns.lisp:3033 is the structural donor â€” the
...
;;; same lane split (AArch64 add-imm is unsigned â€” the adjust-sp /
;;; s61-truncation class). Lane order low-then-high; the first emitted
;;; insn reads src, any second insn reads dest â€” dest=src safe.
;;; const=0 degenerates to (add dest src #0) â€” a copy, correct.
...
;;; his backend (w1 mechanics) â€” force the X views (his u64-popcount /
...
;;; ppc64-vinsns.lisp:3017: (subf dest y x) â€” PPC subf rD,rA,rB is
...
;;; â€” body applies (ash x fixnumshift); w1 U6w gates |x| to 12 bits).
...
;;; constant (one movz/movn lane â€” the 15-bit shifted value always
;;; fits; movn imm = lognot(value), his ratified negative-constant
;;; idiom, cf. v2 arm64-vinsns.lisp:4627) into an unboxed TEMP, then
;;; SUB. v2's body used DEST as the scratch â€” an alias bug when
...
;;; UNSHIFTED)) â€” boxed-src x raw-const preserves boxing; emit site
...
;;; arm64-vinsns.lisp:4728): materialize the raw constant â€” one
;;; movz/movn lane, :s16const always fits â€” and MUL. v2 used DEST as
...
;;; arg_z; w1 U10w flagged the vreg pass-through) â€” separate unboxed
;;; temp here.
...
;;; unboxed before the multiply â€” the v2 shift-0 bare mul
...
;;; â€” line-port. ARM32 arm-vinsns.lisp:3276 same shape. Temp is :s64
;;; (X width; the donors' :s32 would select W templates against X
;;; operands â€” the w1 fixnum-ash correction).
...
;;; %ilsl-c ppc64-vinsns.lisp:2905: (rldicr dest src count ...) â€” shift
...
;;; reg); emit site :834 â€” the variable leg passes an UNBOUNDED count,
...
;;; Mirrored 1:1 â€” cmp against the boxed 63 (504, a definition-time
;;; immediate expression, fits imm12), lsr-unbox, register-form lsl,
;;; b.ls (unsigned <=) skip, else dest = 0. A negative boxed count is
;;; huge unsigned â‡’ takes the 0 path, as on PPC. v2's cmp against raw
...
;;; fixnumshift bits after the shift â€” the w1 report's re-derive note;
...
;;; Out-of-range counts SIGN-FILL (asr by 63), not zero â€” mirrored 1:1
...
;;; Reconcile template ordinals with the assembler's table (same epilogue
;;; as his arm64-vinsns.lisp and the earlier additions files).
(fixup-arm64-vinsn-templates)
...
;;; against HIS kernel"). pins it, and the wait was justified: this is
...
;;; (c) MEASURED on image 44cb19ff63a57dadb4f529e39aca2b95: the literal
...
;;; typecode ∈ {tag-fixnum=0, tag-single-float=1, subtag-bignum=28,
...
;;; 3-bit tag-symbol; his scheme's lisptag #b111 also covers
;;; fulltag-function, so BOTH compares here use the 4-bit fulltag.
...
;;; : the trap was `brk #xf000|arch::error-object-not-list', the w3b
...
;;;
;;; Was `brk #xf0fb', the last placeholder 's UUO swap left because
...
;;; stage 11 at SLOT-UNBOUND.1 and again at WITH-SLOTS ().
...
;;; ============ ksignalerr ============
;;; Emit site: w3 typechecked-form generic leg
;;; (arm642-additions-w3.lisp:1008; args nargs+arg_x/y/z set up by the
;;; handler, $XWRONGTYPE protocol). PPC64 ppc64-vinsns.lisp:4071:
;;; (define-ppc64-subprim-call-vinsn (ksignalerr) .SPksignalerr).
;;; Body = the w1 subprim-call shape (his call-subprim-1/2 idiom: movz
;;; offset / ldr entry off rnil / blr), offset resolved at expand time
;;; by name — .SPksignalerr is in the PROPOSED subprims extension
;;; (arm64-subprims-additions.lisp:51, w2
;;; U5v-class ratify item; loud expand-time failure if absent). Scratch
;;; imm1, not imm0, per the w1 U2 family-uniformity rule (imm0 is a live
;;; subprim ABI input at several sites).
...
;;; odd because relative to the #b0111-tagged pointer â€” LDUR takes it
;;; and the effective address is 8-aligned). Fixnums (tag 0), NIL, and
;;; any unbound-marker non-function all fail the first compare â€” logic
...
;;; ============ cons ============
;;; Emit site: arm642-cons (arm642-additions-w4.lisp) â€” the gate-25
...
;;; @8b1ed24), copied instruction-for-instruction â€” the order (tagged
...
;;; cons.car = +5 off the fulltag-cons-tagged pointer â€” neither
;;; str-scalable â‡’ STUR. cons.size - fulltag-cons = 13 (sub imm12).
;;; Tag clear: BIC-immediate does not exist in A64 (his assembler has
;;; only the shifted-register bic â€” gate-26 "no template matched");
...
;;; written as the UNSIGNED 64-bit value (ldb wrap â€” CL lognot 15 is
...
;;; :729/:731/:733 â€” the general call-fn case where the callee is not
...
;;; time (w1 heap-cons-rest-arg precedent, U3/U4) â€” all three are in the
...
;;; sites).
;;; âš  CALLEE ABI (boot- root): codegen produces the callee in NFN
...
;;; adapts with `mov temp0, nfn` â€” ratify the register with Matt.
...
;;; tail-funcall-vsp â€” 4th family member (PPC64 3969), missed at the
...
;;; fix: p2 stages the callee in NFN, but kernel _SPmvpass tails
;;; into _SPfuncall which dispatches on TEMP0 (PPC do_funcall contract) â€”
...
;;; subprim calls (.SPmisc-alloc: arg_y=element-count, arg_z=subtag â†’
...
;;; vcell-set carries NO write barrier on PPC64 â€” the barrier'd path is
...
;;; order: closed is stored via dest only AFTER dest is formed â€”
...
;;; trap-unless-double-float â€” doubles are BOXED miscobjs (unlike the
...
;;; PPC64 ppc64-vinsns.lisp:3979/3983 (.SPstkconslist{,-star}) â€”
...
;;; complex-single-float->heap â€” box a csf (one 64-bit payload word)
...
;;; â€” computed inline as make-vheader's expansion
;;; (logior subtag (ash element-count 8)); make-vheader is a MACRO in
;;; his arch.lisp (not a defun), so it cannot be (:apply)'d â€” use the
...
(define-arm64-vinsn complex-single-float->heap (((dest :lisp))
 ...
 ;; bytes) â€” x8664 setup-complex-single-float-allocation canon
...
;;; complex-double-float->heap â€” cut-9's sibling: box a cdf (both
...
;;; imagpart) â€” the pad word 16-aligns realpart (arch comment @261).
;;; element-count 6: cdf is ivector-class-32-bit (6 x 4 = 24 data
;;; bytes) â€” x8664 setup-complex-double-float-allocation canon
...
;;; Q-form stur, and a bare 128-bit val matches nothing â€” the original
;;; single (stur val â€¦) here was an arm642-expand-vinsn "unhandled
...
;;; trap-unless-single-float â€” singles are IMMEDIATES in his scheme
;;; (fulltag-single-float #b0001, "and nothing but"; the #b1001 slot is
;;; reserved-not-live), so this is a pure fulltag check â€” no subtag
...
;;; spread-list (PPC64 3973, .SPspreadargz â€” already registered) and
;;; req-heap-rest-arg (PPC64 4037, .SPreq-heap-rest-arg â€” spentry-D
...
;;; save-lisp-context-lexpr â€” PPC64 ppc64-vinsns.lisp:3502 (like the
...
(define-arm64-vinsn save-lisp-context-lexpr (()
 ...
 ;; 1v path) â€” PPC64's `std loc_pc savelr(sp)', NOT lr: lr here is
 ;; the return-to-prologue from the subprim blr, and storing it made
 ;; the function's return jump back into its own prologue (
 ;; infinite vector-allocation loop, disasm-observed). CONSTRAINT:
 ;; temp4 must survive from save-lexpr-argregs to here â€”
 ;; copy-lexpr-argument sits between them for num-fixed > 0 lexprs;
 ;; RESOLVED : the unwired :lisp pool DOES include temp4
...
;;; get-complex-double-float â€” x8664's 128-bit load; his template
...
;;; trap-unless-complex-double-float â€” sibling of the csf trap above
...
;;; slide-values â€” PPC64 ppc64-vinsns.lisp:4015 subprim call .SPmvslide
;;; (kernel body spentry-D mvslide@1532 ends in ret; registered).
;;; MUST link (blr): donor is define-ppc64-subprim-call-vinsn (bla); a br
;;; here made mvslide's ret warp to the caller's stale lr ( fault).
...
;;; save-lexpr-argregs â€” PPC64 ppc64-vinsns.lisp:3525 LINE-PORT.
;;; vpush the received register args (arg_x/y/z) under min-fixed
;;; predicates, push the extra-args count, entry-vsp (WIRED imm0 â€” the
...
;;; mirror cmpldi. nargs fixnums â‡’ (ash n 3) byte constants.
;;; 3 = $numarm64argregs, INLINED (arm64env loads later â€” cut-3 lesson).
(define-arm64-vinsn (save-lexpr-argregs :call :subprim)
 ...
 ;; alias it. ( third spin: without this + the savelr store
 ;; below, a compiled lexpr's frame savelr was the return-to-prologue
 ;; address â€” returning re-entered the function forever.)
  (mov temp4 lr)
...
;;; scale-nargs â€” PPC64 ppc64-vinsns.lisp:2008: for &lexpr/&rest entry,
...
;;; single->node â€” donor x8664-vinsns.lisp:2007 (bits<<32 | tag): his
...
;;; macptr->stack â€” PPC64 ppc64-vinsns.lisp:2773: build a stack-consed
;;; MACPTR on the temp stack (48-byte frame: backlink + zero word +
;;; header/address/domain/type; domain/type MUST be zeroed â€” tstack
;;; memory isn't 0-filled). Frame protocol = our V3b make-stack-cons
;;; canon (str-preindex backlink, RATIFY tstack-frame-shape item).
;;; Displacements land 8-aligned (16 + fulltag-misc + slot offsets â‡’
...
;;; default-optionals â€” PPC64 ppc64-vinsns.lisp:3695: imm0 = the boxed
...
;;; get-complex-single-float â€” donor x8664-vinsns.lisp:1980 (one 64-bit
;;; load of both S lanes from complex-single-float.realpart); emit site
;;; is HIS arm642.lisp:1200. realpart = misc-data-offset = -4 â‡’ D-form
;;; LDUR (w3a misc-ref-c-complex-single-float precedent â€” lanes land
;;; realpartâ†’S[0], imagpartâ†’S[1] on little-endian).
...
;;; (demand-scan cut 2: call-subprim-3 Ã—3 files, save-lisp-context-
;;; variable Ã—2, call-label, setq-special, trap-unless-{complex-single-
;;; float,macptr}.)

;;; call-subprim-3 â€” 3-node-arg member of his call-subprim-1/2 family.
...
;;; NOT imm0 â€” his -1/-2 wire imm0 while their emit sites pass an
...
;;; save-lisp-context-variable â€” donor: ARM32 arm-vinsns.lisp:3630 (no
;;; PPC64 analog by this name; PPC64's save-lisp-context-vsp is the
;;; fixed-vsp cousin). savevsp = vsp + max(0, nargs - argregsÂ·8): when
...
;;; off fn â€” prologue gap in his tree.
(define-arm64-vinsn save-lisp-context-variable (()
 ...
 ;; this file â€” referencing it here aborted the whole file's load
 ;; (demand-scan cut-3 regression). His arch defines no nargregs
 ;; constant Lisp-side (his kernel asm's `nargregs` is .s-only) â€”
 ;; MAIL nit.
...
;;; call-label â€” PPC64 ppc64-vinsns.lisp:2184 verbatim (bl label).
(define-arm64-vinsn (call-label :call) (()
                                        ((label :label)))
  (bl label))


;;; setq-special â€” PPC64 ppc64-vinsns.lisp:3143 (bla .SPspecset);
...
;;; trap-unless-macptr â€” PPC64 ppc64-vinsns.lisp:1589; two-step
;;; typecode extraction exactly like w3b's trap-unless-typecode=
;;; (fulltag; misc â†’ header subtag byte), trap unless subtag-macptr.
;;; arch::error-object-not-macptr exists (arch.lisp enum) â€” named brk.
...
;;; trap-unless-complex-single-float â€” donor x8664-vinsns.lisp:849
;;; (same two-step shape; its uuo carries subtag-complex-single-float
;;; as the expected-type byte â€” exactly his wrong_type udf namespace).
...
;;; ref-symbol-value â€” PPC64 ppc64-vinsns.lisp:3099 (bla .SPspecrefcheck):
...
;;; save-lisp-context-offset â€” PPC64 ppc64-vinsns.lisp:3482 (frame push
...
;;; (mov fn nfn) per PPC64:3490 â€” the "his lambda path does it
...
;;; the first fn-relative constant ref reads the wrong function â€”
;;; frontier fault (fn=0 at lr=ret1val_addr).
...
;;; toplevel-vsp wall (unwind-protect cleanup frame built UP,
;;; clobbering caller frames) â€” 2026-07-17 boot-validated.
(define-arm64-vinsn save-lisp-context-offset (()
                                              ((nbytes-vpushed :u16const))
                                              ((marker-reg :imm)
                                               (vsp-reg :imm)))
 ;; w10 fix: full u16 window for the vsp-reg add (two-lane; nbytes â‰¥ 0,
...
;;; (.SPfitvals: imm0 = requested byte count â€” cont-71 ABI note â€” pads/
...
;;; ============ adjust-vsp ============
;;; Gate-29 demand. PPC64 ppc64-vinsns.lisp:2793 (la vsp amount vsp) â€”
...
;;; pattern â€” vsp chains, order-safe).
...
;;; (() ()) (ld tsp 0 tsp))
;;; â€” pop the temp-stack frame by loading the BACKLINK, which every
;;; tstack frame stores at [tsp, #0] (our V3b make-stack-cons writes it
;;; with (str tsp (:@! tsp (:$ -32))); same PPC64 stdu protocol â€”
...
;;; an 8-aligned address ARE the address (the PPC64 pun â€” v2's
...
;;; misc-data-offset=-4 are 8k-4 â€” POSITIVE and unaligned (empirical:
;;; catch-frame.db-link=20, l1-lisp-threads UOFF3 wall). Scaled LDR
;;; (uoff3, reach 32760) requires offset â‰¥ 0 AND 8-aligned; everything
;;; else takes LDUR (simm9, reach -256..255) â€” the handlers gate their
...
;;; (arm64-arch.lisp:377) â€” far beyond simm9 and not 8-aligned, so the
...
;;; FIX (mechanism gdb-observed): his nrs-offset values are
...
;;; misc-header-offset = -12 â‡’ STUR. Emit sites gate the size within
...
;;; ⚠️ RUNTIME LANDMINE (DECISION-LOG cont-62 2026-06-30): in v2 these
...
;;; SPECIFIER-TYPE wall (gdb-observed: ldur [sym,#4] returned
...
;;; ============ fpr↔fpr copies (4) ============
;;; PROMOTED (2026-07-28) into upstream patch 0061, which defines
...
;;; CORRECTION: the original w9 body passed NIL through raw (a
...
;;; that walked over T ( sysutils $XWRONGTYPE fatal).
...
;;; Spentry draft: spentry-D-call-builtins.s:317.
...
;;;
;;; : these were the vinsns behind acode-handler-coverage.py's
...
;;; FLET.20 probe: threshold exactly 512). Same movz constraint:
...
;;; (arm64-vinsns.lisp:18, "this range is limited"). FLET.20:
...
;;; +32 mirror here was THE toplevel-vsp wall: nthrow1value calls
...
;;; Requires .SPbind registered in arm64-subprims-additions.lisp (w11) â€”
...
;;; dpayback â€” unwind N special-variable bindings on exit from a binding
...
;;; PPC's `li temp n` â€” positive-count movz per the ratified :s16const
...
;;; src) :got-it
;;; The check is the "reconstruct and compare" reconstruction â€” VERBATIM
...
;;; sbfx fixnumshift,32 with sign-extension into the X view) â€” the
...
;;; (ldrsw (:x dest) ...) â€” the X reg ends up holding the 64-bit
...
;;; fixnumshift = 0 and skips the unshift â€” NOT the donor here.
...
;;; sign-extend-halfword â€” demanded by the %word-to-int handler
...
;;; %current-frame-ptr â€” demanded by the %current-frame-ptr handler.
;;; PPC64 LINE-PORT (ppc64-vinsns.lisp:3398): (mr dest ppc::sp) â€” the
...
;;; definition in Matt's arm64-vinsns.lisp (unverifiable locally) â€”
...
;;; mem-ref-c-address / mem-ref-address â€” demanded by the
...
;;; unbox-u64 â€” PPC64 LINE-PORT (ppc64-vinsns.lisp:1306): value = a
...
;;; â€” same fixnummask TST, fulltag-misc test, header LDUR at
;;; misc-header-offset (-12 post-8b1ed24), two/three-digit-bignum-header
;;; compares (all constants resolve in his arch via w3b) â€” with the
...
;;; (-4) read as ONE little-endian 64-bit word ARE the u64 â€” plain
...
;;; unbox-u64 (arm64-vinsns.lisp:2227) â€” wrong layout for this tree.
...
;;; vinsn: CCL::UNBOX-S64" (): his arm64-vinsns.lisp:339 has a draft
...
;;; unbox-s16 â€” PPC64 LINE-PORT (ppc64-vinsns.lisp:1410): the width-16
;;; member of the unbox-s32 family above â€” check body verbatim
...
;;; scale-64bit-misc-index â€” PPC64 LINE-PORT (ppc64-vinsns.lisp:62):
...
;;; corrections_fixnumshift_vs_node_shift â€” here the null shift is the
...
;;; (-4) â‡’ addi renders as SUB of its negation, the family idiom
...
;;;
;;; : this is the vinsn arm64-vinsns-additions-w3a.lisp:445 already
...
;;; set-constant-msb0-bit-to-1 â€” donor PPC64 ppc64-vinsns.lisp:1945
;;; set-constant-ppc-bit-to-1: (oris/ori #x8000>>â€¦) sets PHYSICAL bit
;;; (31 - bitnum) â€” IBM MSB0 numbering, correct on BIG-endian where
...
;;; 1 << bitnum) â€” the read and set sides agree. Name keeps the
...
;;; u64->integer â€” the boxing INVERSE of unbox-u64 above. PPC64 donor
;;; ppc64-vinsns.lisp:2543 â€” an INLINE fixnum-or-bignum path (NOT a
...
;;; âš  DONOR BUG, not reproduced (comment-gold class): the donor's
;;; :three leg stores `header` STILL HOLDING two-digit-bignum-header
;;; (its second `li` is missing) â€” a u64 â‰¥ 2^63 would box as a NEGATIVE
...
;;; dropped â€” the raw u64 stored as one LE 64-bit word at
...
;;; Raw u64 lives ONLY in :u64 imm-class regs (src/header) â€” never a
...
;;; ref-interrupt-level â€” read the current interrupt level (a fixnum)
...
;;; unbox-s8 â€” PPC64 LINE-PORT (ppc64-vinsns.lisp:1442): the width-8
...
;;; %unbox-u16 â€” width-16 sibling of w1's %unbox-u8 (:68): unchecked
...
;;;
;;; arm64-vinsns-additions-w13.lisp â€” wave 13: the aapcs64-ff-call
...
;;; collides â€” checked against w1-w12 2026-07-16). Donor lineage: v2
;;; compiler/ARM64/arm64-vinsns.lisp:3561-3696 (the battle-tested
;;; AAPCS64 family â€” fd-lseek/strace-verified arg marshaling) recut
;;; onto HIS frame layout below; PPC64 ppc64-vinsns.lisp:2270-2348 is
;;; the logic lineage of that donor.
;;;
;;; ============ c-frame layout (HIS, verified ) ============
;;; ⚠️ This block used to describe {backlink@0, savelr@8}. That was WRONG and
...
;;; The kernel side is 0003 (_struct c_frame) and the
;;; canonical note is `spentry ffcall' in arm64-spentry.s.
...
;;; [sp+80] overflow (stack) argument words, contiguous â€” .SPffcall
...
;;; frame owned by .SPffcall as unboxed â€” same obligation his
...
;;; load at offset+0 (PPC's +4 was big-endian) â€” v2 donor deviation
...
;;; ============ ff-call ============
;;; Call .SPffcall (his defsubprim, arm64-arch.lisp:530 â€” already in
;;; the registered table, no subprims-additions entry needed).
;;; Contract (msg-29): C-function address in arg_z (node: macptr or
;;; fixnum-locative â€” subprim extracts the raw pointer); frame as
...
;;; âš  KERNEL GAP: _SPffcall body not yet written for his kernel â€”
...
;;; external-entry-point gvector, trap if NIL (unresolved eep â€” PPC's
;;; tdeqi rendered as cmp/brk per the lane trap registry; #xf0fd =
;;; unresolved-eep, first use). Slot offset -4+8 under HIS
;;; misc-data-offset=-4 â‡’ LDUR (w8 node-slot-ref idiom).
;;; : dest was the donor's class `t` referenced BARE â€” his emitter
...
;;; #_memset spin). Class-t operands need the (:x â€¦) view (w1 load-t
;;; rule); a node dest is :lisp anyway â€” bare :lisp in ldur/cmp is the
...
;;; and extending the table is a kernel rebuild â€” so inline it, the way
...
;;; plain add â€” no shifted-reg or reg-offset addressing modes (encode
...
;;; : this was the ONLY member of the macptr bit family that reused its
...
;;; Demand (, pin advance to 33e61e6): his arm642-ivector-typecode-p
...
;;; that is the lesson restated -- Matt's subtags are not
...
;;; Demand (): his bit-vector %aset1/%set-bit path emits all four
