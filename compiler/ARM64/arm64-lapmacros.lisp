...
;;; ===================================================================
;;; SECTION 5: Fixnum operations
;;; ===================================================================

;;; box-fixnum: NOW UPSTREAM (Matt's commit 8e6a295, arm64-lapmacros.lisp:43)
;;; with the identical body `(lsl dest src fixnumshift)` — removed here to
;;; avoid a duplicate definition.

;;; set-nargs: NOW UPSTREAM (Matt's arm64-lapmacros.lisp @6b6540e) as
;;; `(movz nargs (:$ ',n))` — the quoted immediate boxes to a fixnum in his
;;; assembler, same semantics as our shifted form. Removed here so his
;;; canonical version wins (this file loads AFTER his).

;;; call-subprim: OVERRIDES Matt's (@6b6540e) — his emits
;;; `(add imm0 rnil offset); (blr imm0)`, i.e. jumps to the TABLE ADDRESS
;;; itself off the wrong base (boot- root: the sptab lives at
;;; [rcontext + tcr.sptab + n*8]; the entry must be LOADED). Mail-pile
;;; ratify item alongside his call-subprim-1/2 vinsns. Scratch = imm1,
;;; never imm0 (subprim ABI inputs ride in imm0 — w1/U2, v2 cont-71).
...
;;; load-constant: NOW UPSTREAM (Matt's arm64-lapmacros.lisp @6b6540e) —
;;; his version computes (arm64::constant-offset constant) and emits a real
;;; fn-relative ldur (with load-indexed-constant for the far case), matching
;;; the boot-validated {cv@0, consts@1..} function shape (patch 0009).
;;; Our speculative quoted-immediate form removed so his wins.

;;; load-nfn-constant — NFN-relative twin of his load-constant (v2
;;; s92 cont-13 class, re-observed in EQUAL): a lap function runs
...
;;; (: EQUAL's hairy-equal load fetched %HASH-PROBE's KEY symbol).
...
;;; get-double-float: NOW UPSTREAM (Matt's commit 8e6a295,
;;; arm64-lapmacros.lisp:46) with the identical body
;;; `(ldur dest (:@ node (:$ double-float.value)))` — removed here.
...
;;; digit-h / digit-l: NOT needed. Their only users are the PPC32 LAP arms
;;; (%code-vector-last-instruction LAP body is #+ppc32-target; the PPC64
;;; version is a plain defun. %sfloat-hwords is ppc32-only). The original
;;; inventory misattributed the guard (it sat above a comment block);
;;; tools/level0-port-inventory.py has been fixed and no longer lists them.
(provide "ARM64-LAPMACROS-ADDITIONS")
